# setup ----
setwd("/Users/matthewtarduno/Desktop/212/Section05")
options(scipen=999)
...
# note, we would still need to do this: (x <- mutate(x, b = as.numeric(b)))

# Define our function
# indicies you want to replace, what you want to repace them with. 
rep_na <- function(y) replace(y, which(y == ".." | y=="-"), NA) #replace dots or dashes with NA's. 

# Make some data
x <- data.frame(a = c(1:4, NA), b = c("..", 1, 2, 3, ".."))  %>% as_tibble()
# Replace .. with NA in column b 
x %>% mutate(b = rep_na(b) %>% as.numeric())

# we can also apply this to all columns: 

# Make some data
x <- data.frame(a = c(1:4, NA), b = c("..", 1, 2, 3, "-"))  %>% as_tibble()
# Replace .. with NA
x %>% mutate_all(rep_na) %>% mutate_all(as.numeric)

# section notes also contain info on polynomials 





# A little tldr on pipes ----

# still confused about pipes? Read this: http://r4ds.had.co.nz/pipes.html
...
# Directories?
dir_data <- "/Users/matthewtarduno/Desktop/212/Section05/"
...
# magrittr offers is more flexibility for piping commands
...
# Aew pipe! %<>% (updater pipe) (from Magrittr) ----

# assignment pipe  / bidirectional pipe

# Another new pipe "%$%" -----

# More simply: %$% helps you avoid using too many $s
# want more? Use vignette("...")
vignette("magrittr")
# browseVignettes() # available ones (careful)

cor(cars$price, cars$weight)
## [1] 0.5386115
cars %$% cor(price, weight) #wow this is dumb 
## [1] 0.5386115
...
# pt(q, df), which gives the distribution function for a t-distributed 
# random variable evaluated at q with df degrees of freedom
...
pt(q = abs(t_stats), df = n-k, lower.tail = F) * 2 #probably won't work 
...
# this function tests that all the coefficients are different from zero 

# pf replaces pt, now we use F-distribution 
...
joint_test(data = cars, y_var = "price", X_vars = c("mpg", "weight"))

# test (yay!)
felm(price ~ mpg + weight, cars) %>% summary() %$% F.fstat
...
ols_joint(data = cars, y_var = "price", X_vars = c("mpg", "weight"), intercept = T)

# and our warning! 
ols_joint(data = cars, y_var = "price", X_vars = c("mpg", "weight"), intercept = F)
...
# Parallelized ols_sim function ('mcapply' replaces 'replicate')
