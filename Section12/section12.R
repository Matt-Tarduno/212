...
# My directory
dir_12 <- "/Users/matthewtarduno/Desktop/212/Section12/"
...
# Slot names 
# (this is important -- structure includes data, polygons, plotorder, bbox, proj4string)
# bbox is bounding box 
# What map projection if proj4
...
# 'bbox' slot (bounding box in lat, long)
beats_shp@bbox
...
# Class of the first element in the 'polygons' slot (still a polygon)
beats_shp@polygons[[1]] %>% class()

# Slot names for a polygon (then this has slot names)
beats_shp@polygons[[1]] %>% slotNames()

# What is the class?
beats_shp@polygons[[1]]@Polygons %>% class()

# What is the length? (yay now length is only one)
