...
# for stuff that isn't yet on cran, of on github: 
...
dir_sec6 <- "/Users/matthewtarduno/Desktop/212/Section06/"
# Load data
...
# Add lines (ugly, really)
ggplot(data = cars, aes(x = weight, y = price)) +
  geom_point() +
  geom_line()

# note that these literally draws over the last layer
...
# Change sizes (make the line thicker, points thicker)
ggplot(data = cars, aes(x = weight, y = price)) +
  geom_point(size = 2) +
...
# note the smoother changes based on number of points!!
...
# Remove confidence intervals (use se = F)
ggplot(data = cars, aes(x = weight, y = price)) +
  geom_point() +
...
# CORRECT quadratic by 'foreign' (basically breaks all aestetics into two layers, based on foreign)

# After  doing this, all aesthetics will not break down by cars
