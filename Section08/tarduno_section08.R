...
# Using simulation to show things about asymptotics 
...
# The "normal" way
# ask if X is between two values

(x >= 1) & (x <= 5)


# The "between" way
# ask if x is between 1 and 5. 
...
# Directory
setwd("/Users/matthewtarduno/Desktop/212/Section08/")
# My ggplot2 theme
theme_ed <- theme(
...
# Simulate for population of 100,000 
# Take 10,000 draws of differnet sizes. 
# We will also check for epsilon to have 4 types of distributions (normal, uniform, poisson, bimodal)
...
rbigamma <- function(n, prob = c(0.5, 0.5)) {
  ...
  # okay this is kinda wild (check out below)
...
# how that weird sampling line in rbigamma thing worked: 
...
# Start the population (here we make x, and also make all of the errors)
pop_df <- data.frame(
  ones   = 1,
...
# Calculate the outcome variable: y = 1 + beat * x + error (for each kind of error)
pop_df %<>% mutate(
...
# Combine the plots (This is how you combine in ggplot)
...
# note little n is an argument of this function 

one_iter <- function(n, data) {
  # Draw 'n' observations from 'data'
  tmp_df <- sample_n(tbl = data, size = n)
  
  # Define the X matrix (same across regressions) (this just makes code more efficient)
...
# Uh, don't use this one, see below
